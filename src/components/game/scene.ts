...
export function createAbandonedBuilding(
  x: number,
  y: number,
  type: AbandonedBuildingType,
  scale = 1
): THREE.Group {
  const building = new THREE.Group();
  const mat = createToonMaterial(0x5a5a5a, 0.55, 0.1); // Grey, worn look
  const trimMat = createToonMaterial(0x3a3a3a, 0.6, 0.12);
  const roofMat = createToonMaterial(0x4a4a4a, 0.5, 0.08);

  switch (type) {
    case 'collapsed': {
      // Collapsed building - just a pile of rubble
      const rubbleGroup = new THREE.Group();
      for (let i = 0; i < 15; i++) {
        const size = 0.3 + Math.random() * 0.6;
        const geo = [new THREE.BoxGeometry(size, size, size), new THREE.CylinderGeometry(size/2, size/2, size, 6)][Math.floor(Math.random() * 2)];
      ...
      rubbleGroup.position.set(x, y, 0.12);
      rubbleGroup.rotation.set(Math.random() * 0.2, Math.random() * 0.2, Math.random() * 0.2);
      building.add(rubbleGroup);
      break;
    }

    case 'ruined': {
      // Ruined building - mostly intact but with missing walls and broken windows
      const width = 4, depth = 3, height = 2;
      const wallMat = createToonMaterial(0x6a6a6a, 0.5, 0.08);
      const roofMat = createToonMaterial(0x4a4a4a, 0.5, 0.08);

      // Main structure
      const base = new THREE.Mesh(new THREE.BoxGeometry(width, depth, height), wallMat);
      base.position.set(0, depth / 2, height / 2);
      building.add(base);

      // Missing sections (create gaps)
      const missingWalls = Math.floor(Math.random() * 3) + 2;
      for (let i = 0; i < missingWalls; i++) {
        const side = Math.floor(Math.random() * 4);
        const wx = (side % 2 === 0 ? 0 : (side === 1 ? width/2 : -width/2));
        const wy = (side < 2 ? (side === 0 ? depth/2 : -depth/2) : 0);
        const missing = new THREE.Mesh(
          new THREE.BoxGeometry(side % 2 === 0 ? width : 0.5, side < 2 ? depth : 0.5, height),
          wallMat
        );
        missing.position.set(wx, wy, height / 2);
        building.add(missing);
      }
      ...
      for (let i = 0; i < 3; i++) {
        ...
        const wx = (Math.random() - 0.5) * (width - 0.5);
        const wy = (Math.random() - 0.5) * (depth - 0.5);
        brokenWindow.position.set(wx, wy, height + 0.01);
        building.add(brokenWindow);
      }

      // Damaged roof
      const roof = new THREE.Mesh(new THREE.BoxGeometry(width + 0.4, depth + 0.4, 0.1), roofMat);
      roof.position.set(0, depth / 2, height + 0.08);
      building.add(roof);
      break;
    }

    case 'cracked': {
      // Cracked building - vertical cracks visible
      const width = 3, depth = 2, height = 2.5;
      const wallMat = createToonMaterial(0x5a5a5a, 0.5, 0.08);
      const roofMat = createToonMaterial(0x4a4a4a, 0.5, 0.08);
      ...
      // Cracks (visual only)
      const crackMat = new THREE.MeshBasicMaterial({ color: 0x3a3a3a, transparent: true, opacity: 0.5 });
      for (let i = 0; i < 3; i++) {
        ...
        crack.rotation.set(Math.random() * 0.3, Math.random() * 0.3, Math.random() * 0.3);
        building.add(crack);
      }

      // Partial roof collapse
      const roof = new THREE.Mesh(new THREE.BoxGeometry(width + 0.3, depth + 0.3, 0.08), roofMat);
      roof.position.set(0, depth / 2, height + 0.06);
      building.add(roof);
      break;
    }

    case 'overgrown': {
      // Overgrown building - ivy covering, vegetation
      const width = 4, depth = 3, height = 2;
      const wallMat = createToonMaterial(0x6a6a6a, 0.5, 0.08);
      const roofMat = createToonMaterial(0x4a4a4a, 0.5, 0.08);
      const ivyMat = createToonMaterial(0x2d5a27, 0.6, 0.15); // Dark green
    ...
    case 'collapsed-store': {
      // Collapsed store - storefront still partially standing
      const width = 5, depth = 2, height = 2;
      const wallMat = createToonMaterial(0x5a5a5a, 0.5, 0.08);
      const roofMat = createToonMaterial(0x4a4a4a, 0.5, 0.08);

      // Back wall still standing
      const backWall = new THREE.Mesh(new THREE.BoxGeometry(width, depth, height), wallMat);
      backWall.position.set(0, depth / 2, height / 2);
      building.add(backWall);

      // Front wall collapsed (just storefront frame)
      const frame = new THREE.Mesh(
        new THREE.BoxGeometry(width, 0.1, 0.1),
        wallMat
      );
      frame.position.set(0, -depth / 2, height / 2 + 0.05);
      building.add(frame);

      // A-frame roof collapsed
      const collapsedRoof = new THREE.Mesh(
        new THREE.ConeGeometry(width / 2, 1.5, 4),
        roofMat
      );
      collapsedRoof.position.set(0, -depth / 2, height + 0.75);
      collapsedRoof.rotation.x = Math.PI / 2;
      collapsedRoof.scale.z = 0.5;
      building.add(collapsedRoof);

      // Broken window frame
      const frameWindow = new THREE.Mesh(
        new THREE.BoxGeometry(width - 0.5, 0.08, 0.08),
        wallMat
      );
      frameWindow.position.set(0, -depth / 2 + 0.3, height / 2);
      building.add(frameWindow);
      break;
    }

    case 'ruined-shack': {
      // Small ruined shack
      const width = 2, depth = 1.6, height = 1.5;
      const wallMat = createToonMaterial(0x5a5a5a, 0.5, 0.08);
      const roofMat = createToonMaterial(0x4a4a4a, 0.5, 0.08);
      ...
      // Missing wall on one side
      const missingWall = new THREE.Mesh(
        new THREE.BoxGeometry(width, 0.1, 0.1),
        wallMat
      );
      missingWall.position.set(0, -depth / 2 + 0.2, height / 2 + 0.3);
      building.add(missingWall);

      // Leaning roof
      const roof = new THREE.Mesh(
        new THREE.ConeGeometry(width / 2, 0.8, 4),
        roofMat
      );
      roof.position.set(0, -depth / 2 + 0.4, height + 0.4);
      ...
      for (let i = 0; i < 5; i++) {
        const debris = new THREE.Mesh(
          new THREE.BoxGeometry(0.2 + Math.random() * 0.3, 0.2 + Math.random() * 0.3, 0.2 + Math.random() * 0.3),
          wallMat
        );
        debris.position.set(
          (Math.random() - 0.5) * 1.5,
  ...
  building.position.set(x, y, 0);
  applyShadows(building, true, true);
  return building;
}
...
export function createDebris(
  x: number,
  y: number,
  type: 'metal' | 'concrete' | 'wood' | 'glass' | 'scrap' | 'rope' = 'scrap'
): THREE.Mesh {
  const mats = {
    metal: createToonMaterial(0x6b7280, 0.5, 0.08),
    concrete: createToonMaterial(0x5a5a5a, 0.5, 0.08),
    wood: createToonMaterial(0x8b6b4a, 0.5, 0.08),
    glass: new THREE.MeshBasicMaterial({ color: 0x4a90d9, transparent: true, opacity: 0.3 }),
    scrap: createToonMaterial(0x8b7d6b, 0.5, 0.08),
    rope: createToonMaterial(0x5a5a5a, 0.5, 0.08),
  };

  const geo = {
    metal: new THREE.BoxGeometry(0.1, 0.5, 0.1),
    concrete: new THREE.BoxGeometry(0.3, 0.2, 0.3),
    wood: new THREE.BoxGeometry(0.2, 0.2, 0.8),
    glass: new THREE.PlaneGeometry(0.3, 0.3),
    scrap: new THREE.CylinderGeometry(0.05, 0.08, 0.2, 6),
    rope: new THREE.CylinderGeometry(0.02, 0.02, 0.4, 6),
  }[type];

  const debris = new THREE.Mesh(geo, mats[type]);
  debris.position.set(x, y, 0.1 + Math.random() * 0.1);
  debris.rotation.set(Math.random() * Math.PI, Math.random() * Math.PI, Math.random() * Math.PI);
  debris.scale.set(
    0.5 + Math.random() * 1.6,
    0.5 + Math.random() * 1.6,
...
export function createDebrisCluster(
  x: number,
  y: number,
  count = 8,
  types: ('metal' | 'concrete' | 'wood' | 'glass' | 'scrap' | 'rope')[] = ['scrap', 'metal', 'concrete', 'glass', 'wood']
): THREE.Group {
  ...
  cluster.rotation.set(Math.random() * 0.2, Math.random() * 0.2, Math.random() * 0.2);
  applyShadows(cluster, true, true);
  return cluster;
}

// Create a pile of rubble
export function createRubblePile(
  x: number,
  y: number,
  size = 1.5
): THREE.Group {
  const rubble = new THREE.Group();
  const mat = createToonMaterial(0x5a5a5a, 0.5, 0.08);
  const numRocks = Math.floor(size * 6);

  for (let i = 0; i < numRocks; i++) {
    const rockSize = 0.1 + Math.random() * size * 0.5;
    const rockGeo = [new THREE.BoxGeometry(rockSize, rockSize, rockSize), new THREE.CylinderGeometry(rockSize/2, rockSize/2, rockSize, 6)][Math.floor(Math.random() * 2)];
  ...
  rubble.position.set(x, y, 0.1);
  rubble.scale.set(size, size * 0.6, size);
  applyShadows(rubble, true, true);
  return rubble;
}

// Create broken street furniture
export function createBrokenStreetFurniture(
  x: number,
  y: number,
  type: 'bench' | 'lamp' | 'sign' | 'pole' = 'bench'
): THREE.Group {
  const furniture = new THREE.Group();
  const mat = createToonMaterial(0x6a6a7a, 0.5, 0.08);

  switch (type) {
    case 'bench': {
      // Broken bench - split in two
      const bench1 = new THREE.Mesh(
        new THREE.BoxGeometry(0.4, 0.05, 0.15),
        mat
      );
      bench1.position.set(-0.2, 0.05, 0);
      furniture.add(bench1);

      const bench2 = new THREE.Mesh(
        new THREE.BoxGeometry(0.6, 0.05, 0.15),
        mat
      );
      bench2.position.set(0.2, 0.05, 0.05);
      bench2.rotation.z = (Math.random() - 0.5) * 0.5;
      furniture.add(bench2);
      break;
    }

    case 'lamp': {
      // Broken lamp - pole bent
      const pole = new THREE.Mesh(
      ...
      const lampHead = new THREE.Mesh(
        ...
        new THREE.MeshBasicMaterial({ color: 0x4a4a4a, transparent: true, opacity: 0.5 })
      );
      lampHead.position.set(0, 1.5, 0);
    ...
    case 'sign': {
      ...
      sign.rotation.z = Math.PI / 2 + (Math.random() - 0.5) * 0.3;
      furniture.add(sign);
      break;
    }
    ...
    case 'pole': {
      ...
      pole1.position.set(-0.15, 0.4, 0);
      pole1.rotation.z = (Math.random() - 0.5) * 0.3;
      furniture.add(pole1);
      ...
      pole2.position.set(0.15, 0.4, 0);
      pole2.rotation.z = (Math.random() - 0.5) * 0.3;
      furniture.add(pole2);
      break;
    }
  }

  furniture.position.set(x, y, 0.1);
  applyShadows(furniture, true, true);
  return furniture;
}

// Create a port/terminal
export function createPortTerminal(
  x: number,
  y: number,
  scale = 1
): THREE.Group {
  const port = new THREE.Group();
  const metalMat = createToonMaterial(0x64748b, 0.5, 0.08);
  const accentMat = createToonMaterial(0x3b82f6, 0.6, 0.12); // Blue accent
  const darkMat = createToonMaterial(0x1e293b, 0.5, 0.08);
  ...
  platform.position.set(0, 0, 0.08);
  port.add(platform);
  ...
  for (let i = -1; i <= 1; i++) {
    ...
    pillar.position.set(i * 1.3, 1.25, 0.08);
  ...
  for (let i = -1; i <= 1; i++) {
    ...
    panel.position.set(i * 1.3, 2.5, 0.12);
    port.add(panel);
    ...
    screen.position.set(i * 1.3, 2.5, 0.13);
    port.add(screen);
  }
  ...
  antenna.position.set(0, 3.55, 0.08);
  ...
  beacon.position.set(0, 3.55, 0.08);
  beacon.userData.animated = 'beacon';
  port.add(beacon);

  port.position.set(x, y, 0);
  applyShadows(port, true, true);
  return port;
}
